...
OUTPUT_DIR = "outputs/predictions_v2"
...
# --- Request limits -----------------------------------------------------------
GROQ_MAX_TOKENS    = 1200      # raised from 1200 -- per-horizon output is larger
CLAUDE_MAX_TOKENS  = 8000      # extended thinking + JSON output needs headroom
MAX_RETRIES        = 5
...
def run(max_news: int = MAX_NEWS) -> list:
    ...
    """
    print("=" * 62)
    print("  EGX30 News-Impact Pipeline (v2: 2-agent, TA-grounded)")
    print(f"  Articles to process : {max_news if max_news is not None else 'ALL'}")
    print(f"  Groq model          : {GROQ_MODEL}")
    print(f"  Claude model        : {CLAUDE_MODEL}")
    print(f"  Groq  max tokens    : {GROQ_MAX_TOKENS}")
